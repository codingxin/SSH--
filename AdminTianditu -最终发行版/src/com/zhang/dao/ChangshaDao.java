...
public class ChangshaDao {
	...
	public boolean delete(int id) {
		Tianditu changsha = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(changsha);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_changsha) {
		...
		StringBuffer sb = new StringBuffer("from Tianditu where place='长沙市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> changshaList = q.list();
		tx.commit();
		session.close();
		return changshaList;
	}
