...
public class LoudiDao {
	...
	public boolean delete(int id) {
		Tianditu Loudi = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(Loudi);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_Loudi) {
		...
		//条件查询
		StringBuffer sb = new StringBuffer("from Tianditu where place='娄底市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> LoudiList = q.list();
		tx.commit();
		session.close();
		System.err.println(LoudiList);
		return LoudiList;
	}
