...
public class XiangtanDao {
	...
	public boolean delete(int id) {
		Tianditu Xiangtan = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(Xiangtan);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_Xiangtan) {
		...
		//条件查询
		StringBuffer sb = new StringBuffer("from Tianditu where place='湘潭市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> XiangtanList = q.list();
		tx.commit();
		session.close();
		System.err.println(XiangtanList);
		return XiangtanList;
	}
