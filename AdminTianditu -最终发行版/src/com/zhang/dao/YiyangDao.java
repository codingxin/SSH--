...
public class YiyangDao {
	...
	public boolean delete(int id) {
		Tianditu Yiyang = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(Yiyang);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_Yiyang) {
		...
		//条件查询
		StringBuffer sb = new StringBuffer("from Tianditu where place='益阳市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> YiyangList = q.list();
		tx.commit();
		session.close();
		System.err.println(YiyangList);
		return YiyangList;
	}
