...
public class YueyangDao {
	...
	public boolean delete(int id) {
		Tianditu yueyang = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(yueyang);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_Yueyang) {
		...
		StringBuffer sb = new StringBuffer("from Tianditu where place='岳阳市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> YueyangList = q.list();
		tx.commit();
		session.close();
		return YueyangList;
	}
