...
public class ShaoyangDao {
	...
	public boolean delete(int id) {
		Tianditu Shaoyang = this.findById(id);
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tr = session.beginTransaction();
		session.delete(Shaoyang);
		tr.commit();
		session.close();
		return true;
	}
	...
	public List<Tianditu> find(PageBean pageBean, Tianditu s_Shaoyang) {
		...
		//条件查询
		StringBuffer sb = new StringBuffer("from Tianditu where place='邵阳市' order by update_time");
		Session session = getHibernateTemplate().getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(sb.toString());
		q.setFirstResult(pageBean.getStart());
		q.setMaxResults(pageBean.getPageSize());
		List<Tianditu> ShaoyangList = q.list();
		tx.commit();
		session.close();
		System.err.println(ShaoyangList);
		return ShaoyangList;
	}
