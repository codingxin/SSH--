			...
			<div class="box">
				...
				<div class="box-body table-responsive no-padding">
					<table class="table table-hover table-bordered text-center">
						...
						<c:if test="${LoudiList!=null }">
							<%--  varStatus是<c:forEach>jstl循环标签的一个属性，varStatus属性。 例如实现隔行变色标志--%>
							<c:forEach var="member" items="${LoudiList }"
								varStatus="status">
								<tr>
		...
		$(function() {
			...
			$(".del")
					.click(
							function() {
								...
								if (confirm("确认要删除吗？")) {
									var id = $(this).data("id");
									$.ajax({url : "/AdminTianditu/loudi/del",
												type : "POST",
												data : {
													id : id
												},
												...
												success : function(data) {
													...
													if (data.success) {
														//菜单栏当前选中
														window.location.href = "/AdminTianditu/loudi/showList";
			...
			$member_update_form
					.submit(function() {
						...
						$
								.ajax({
									url : "/AdminTianditu/loudi/update",
									type : "POST",
									dataType : "json",
									data : md,
									beforeSend : function() {
